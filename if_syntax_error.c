...
void print_syntax_error(core_data *crdata, char *input, int i, int bool)
{
	...
	msg2 = ": Syntax error: \"";
	msg3 = "\" unexpected\n";
	counter = aux_itoa(crdata->counter);
	length = _strlen(crdata->av[0]) + _strlen(counter);
...
/**
 * if_syntax_error - intermediate function to
...
int if_syntax_error(core_data *crdata, char *input)
{
	...
	if (f_char == -1)
	{
		print_syntax_error(crdata, input, begin, 0);
		return (1);
	}
	...
	if (i != 0)
	{
		print_syntax_error(crdata, input, begin + i, 1);
		return (1);
	}
